 ...
 * </PRE>
 * 
 * This will let you do Graphics2D calls directly, but is not supported in any
 * way shape or form. Which just means "have fun, but don't complain if it
 * breaks."
 * <p>
 * Advanced <a href=
...
//  }
	...
	/*
	 * @Override protected void allocate() { // Tried this with RGB instead of ARGB
	 * for the primarySurface version, // but didn't see any performance difference
	 * (OS X 10.6, Java 6u24). // For 0196, also attempted RGB instead of ARGB, but
	 * that causes // strange things to happen with blending. // image = new
	 * BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB); if
	 * (primarySurface) { if (useCanvas) { if (canvas != null) {
	 * parent.removeListeners(canvas); parent.remove(canvas); } canvas = new
	 * Canvas(); canvas.setIgnoreRepaint(true);
	 * 
	 * // parent.setLayout(new BorderLayout()); // parent.add(canvas,
	 * BorderLayout.CENTER); parent.add(canvas); // canvas.validate(); //
	 * parent.doLayout();
	 * 
	 * if (canvas.getWidth() != width || canvas.getHeight() != height) {
	 * PApplet.debug("PGraphicsJava2D comp size being set to " + width + "x" +
	 * height); canvas.setSize(width, height); } else {
	 * PApplet.debug("PGraphicsJava2D comp size already " + width + "x" + height); }
	 * 
	 * parent.addListeners(canvas); // canvas.createBufferStrategy(1); // g2 =
	 * (Graphics2D) canvas.getGraphics();
	 * 
	 * } else { parent.updateListeners(parent); // in case they're already there
	 * 
	 * // using a compatible image here doesn't seem to provide any performance
	 * boost
	 * 
	 * if (useOffscreen) { // Needs to be RGB otherwise there's a major performance
	 * hit [0204] // http://code.google.com/p/processing/issues/detail?id=729 image
	 * = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB); //
	 * GraphicsConfiguration gc = parent.getGraphicsConfiguration(); // image =
	 * gc.createCompatibleImage(width, height); offscreen = new BufferedImage(width,
	 * height, BufferedImage.TYPE_INT_ARGB); // offscreen =
	 * gc.createCompatibleImage(width, height); g2 = (Graphics2D)
	 * offscreen.getGraphics();
	 * 
	 * } else { // System.out.println("hopefully faster " + width + " " + height);
	 * // new Exception().printStackTrace(System.out);
	 * 
	 * GraphicsConfiguration gc = canvas.getGraphicsConfiguration(); // If not
	 * realized (off-screen, i.e the Color Selector Tool), // gc will be null. if
	 * (gc == null) { GraphicsEnvironment ge =
	 * GraphicsEnvironment.getLocalGraphicsEnvironment(); gc =
	 * ge.getDefaultScreenDevice().getDefaultConfiguration(); }
	 * 
	 * image = gc.createCompatibleImage(width, height); g2 = (Graphics2D)
	 * image.getGraphics(); } } } else { // not the primary surface // Since this
	 * buffer's offscreen anyway, no need for the extra offscreen // buffer.
	 * However, unlike the primary surface, this feller needs to be // ARGB so that
	 * blending ("alpha" compositing) will work properly. image = new
...
//  Graphics2D g2old;
	...
	public Graphics2D checkImage() {
		if ((this.image == null) || (((BufferedImage) this.image).getWidth() != (this.width * this.pixelDensity))
				|| (((BufferedImage) this.image).getHeight() != (this.height * this.pixelDensity))) {
...
//      }
			...
			// P.S.: Three years later, I'm happy to report it did in fact hurt [jv
			// 2018-06-01]
...
//      System.out.println("re-creating image");
			...
			this.image = new BufferedImage(wide, high, BufferedImage.TYPE_INT_ARGB);
		}
	...
	public void beginDraw() {
		...
		if (this.fontObject != null) {
			this.g2.setFont(this.fontObject);
		}
		 ...
		 * and 2) minimal overhead, however. Instinct suggests #1 may be true, // but #2
		 * seems a problem. if (primarySurface && !useOffscreen) { GraphicsConfiguration
		 * gc = canvas.getGraphicsConfiguration(); if (false) { if (image == null ||
		 * ((VolatileImage) image).validate(gc) == VolatileImage.IMAGE_INCOMPATIBLE) {
		 * image = gc.createCompatibleVolatileImage(width, height); g2 = (Graphics2D)
		 * image.getGraphics(); reapplySettings = true; } } else { if (image == null) {
		 * image = gc.createCompatibleImage(width, height);
		 * PApplet.debug("created new image, type is " + image); g2 = (Graphics2D)
		 * image.getGraphics(); reapplySettings = true; } } }
		 * 
		 ...
		 * BufferCapabilities caps = strategy.getCapabilities(); caps =
		 * strategy.getCapabilities();
		 * PApplet.debug("PGraphicsJava2D.beginDraw() caps are " + " flipping: " +
		 * caps.isPageFlipping() + " front/back accel: " +
		 * caps.getFrontBufferCapabilities().isAccelerated() + " " + "/" +
		 * caps.getBackBufferCapabilities().isAccelerated()); } GraphicsConfiguration gc
		 * = canvas.getGraphicsConfiguration();
		 * 
		 * if (bimage == null || bimage.getWidth() != width || bimage.getHeight() !=
		 * height) { PApplet.debug("PGraphicsJava2D creating new image"); bimage =
		 * gc.createCompatibleImage(width, height); // image = new BufferedImage(width,
		 * height, BufferedImage.TYPE_INT_ARGB); g2 = bimage.createGraphics();
		 * defaultComposite = g2.getComposite(); reapplySettings = true; } }
		 */
	...
	protected void handleSmooth() {
		if (this.smooth == 0) {
			...
			if ((this.smooth == 1) || (this.smooth == 3)) { // default is bicubic
...
//                        RenderingHints.VALUE_ALPHA_INTERPOLATION_QUALITY);
	...
	public void endDraw() {
		...
		if (this.primaryGraphics) {
			 ...
			 * 
			 * // alternate version //canvas.repaint(); // ?? what to do for swapping
			 * buffers
			 * 
			 * // System.out.println("endDraw() frameCount is " + parent.frameCount); // if
			 * (parent.frameCount != 0) { redraw(); // }
			 * 
			 * } else if (useOffscreen) { // don't copy the pixels/data elements of the
			 * buffered image directly, // since it'll disable the nice speedy pipeline
			 * stuff, sending all drawing // into a world of suck that's rough 6 trillion
			 * times slower. synchronized (image) { //System.out.println("inside j2d sync");
			 * image.getGraphics().drawImage(offscreen, 0, 0, null); }
			 * 
			 * } else { // changed to not dispose and get on each frame, // otherwise a new
			 * Graphics context is used on each frame // g2.dispose(); //
...
//    super.updatePixels();
	...
	/*
	 * private void redraw() { // only need this check if the validate() call will
	 * use redraw() // if (strategy == null) return; do {
	 * PApplet.debug("PGraphicsJava2D.redraw() top of outer do { } block"); do {
	 * PApplet.debug("PGraphicsJava2D.redraw() top of inner do { } block");
	 * PApplet.debug("strategy is " + strategy); Graphics bsg =
	 * strategy.getDrawGraphics(); // if (vimage != null) { // bsg.drawImage(vimage,
	 * 0, 0, null); // } else { bsg.drawImage(bimage, 0, 0, null); // if
	 * (parent.frameCount == 0) { // try { // ImageIO.write(image, "jpg", new
	 * java.io.File("/Users/fry/Desktop/buff.jpg")); // } catch (IOException e) { //
	 * e.printStackTrace(); // } // } // } bsg.dispose();
	 * 
	 * // the strategy version // g2.dispose(); // if (!strategy.contentsLost()) {
	 * // if (parent.frameCount != 0) { // Toolkit.getDefaultToolkit().sync(); // }
	 * // } else { // System.out.println("XXXXX strategy contents lost"); // } // }
...
//  }
	...
	/*
	 * protected PShape createShapeImpl(PGraphicsJava2D pg, int type) { PShape shape
	 * = null; if (type == PConstants.GROUP) { shape = new PShape(pg,
	 * PConstants.GROUP); } else if (type == PShape.PATH) { shape = new PShape(pg,
	 * PShape.PATH); } else if (type == PShape.GEOMETRY) { shape = new PShape(pg,
	 * PShape.GEOMETRY); } // defaults to false, don't assign it and make complexity
	 * for overrides //shape.set3D(false); return shape; }
	 */

	/*
	 * static protected PShape createShapeImpl(PGraphicsJava2D pg, int kind,
	 * float... p) { PShape shape = null; int len = p.length;
	 * 
	 * if (kind == POINT) { if (len != 2) {
	 * showWarning("Wrong number of parameters"); return null; } shape = new
	 * PShape(pg, PShape.PRIMITIVE); shape.setKind(POINT); } else if (kind == LINE)
	 * { if (len != 4) { showWarning("Wrong number of parameters"); return null; }
	 * shape = new PShape(pg, PShape.PRIMITIVE); shape.setKind(LINE); } else if
	 * (kind == TRIANGLE) { if (len != 6) {
	 * showWarning("Wrong number of parameters"); return null; } shape = new
	 * PShape(pg, PShape.PRIMITIVE); shape.setKind(TRIANGLE); } else if (kind ==
	 * QUAD) { if (len != 8) { showWarning("Wrong number of parameters"); return
	 * null; } shape = new PShape(pg, PShape.PRIMITIVE); shape.setKind(QUAD); } else
	 * if (kind == RECT) { if (len != 4 && len != 5 && len != 8 && len != 9) {
	 * showWarning("Wrong number of parameters"); return null; } shape = new
	 * PShape(pg, PShape.PRIMITIVE); shape.setKind(RECT); } else if (kind ==
	 * ELLIPSE) { if (len != 4 && len != 5) {
	 * showWarning("Wrong number of parameters"); return null; } shape = new
	 * PShape(pg, PShape.PRIMITIVE); shape.setKind(ELLIPSE); } else if (kind == ARC)
	 * { if (len != 6 && len != 7) { showWarning("Wrong number of parameters");
	 * return null; } shape = new PShape(pg, PShape.PRIMITIVE); shape.setKind(ARC);
	 * } else if (kind == BOX) { showWarning("Primitive not supported in 2D"); }
	 * else if (kind == SPHERE) { showWarning("Primitive not supported in 2D"); }
	 * else { showWarning("Unrecognized primitive type"); }
	 * 
	...
	public void endShape(int mode) {
		...
		if (this.gpath != null) { // make sure something has been drawn
		...
		public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			for (int y = 0; y < height; y++) {
				...
				for (int x = 0; x < width; x++) {
					dstPixels[x] = blendColor(dstPixels[x], srcPixels[x], this.mode);
				}
	...
	public void quadraticVertex(float ctrlX, float ctrlY, float endX, float endY) {
		...
		this.bezierVertex(x1 + (((ctrlX - x1) * 2) / 3.0f), y1 + (((ctrlY - y1) * 2) / 3.0f),
				endX + (((ctrlX - endX) * 2) / 3.0f), endY + (((ctrlY - endY) * 2) / 3.0f), endX, endY);
	}
	...
	protected void curveVertexSegment(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
		...
		this.gpath.curveTo(this.curveDrawX[1], this.curveDrawY[1], this.curveDrawX[2], this.curveDrawY[2],
...
//  }
	...
	protected void imageImpl(PImage who, float x1, float y1, float x2, float y2, int u1, int v1, int u2, int v2) {
		...
		this.g2.drawImage(((ImageCache) this.getCache(who)).image, (int) x1, (int) y1, (int) x2, (int) y2, u1, v1, u2,
				v2, null);
...
//      }
			...
			if (this.image == null) {
				this.image = new BufferedImage(source.pixelWidth, source.pixelHeight, BufferedImage.TYPE_INT_ARGB);
			}
...
//        System.out.println("source.pixels[0] alpha is " + (source.pixels[0] >>> 24));
				...
				if (targetType == RGB) {
					...
					for (int y = 0; y < source.pixelHeight; y++) {
						...
						wr.setDataElements(0, y, source.pixelWidth, 1, this.tintedTemp);
					}
...
//      g.dispose();
	...
	public PShape loadShape(String filename, String options) {
		...
		if ("svg".equals(extension) || "svgz".equals(extension)) {
			return new PShapeJava2D(this.parent.loadXML(filename));
		}
	...
	public float textAscent() {
		...
		if (font != null) {
			...
			return this.g2.getFontMetrics(font).getAscent();
		}
	...
	public float textDescent() {
		...
		if (font != null) {
			...
			return this.g2.getFontMetrics(font).getDescent();
		}
...
//              TextAttribute.TRACKING_TIGHT);
			...
			/*
			 * Map<TextAttribute, ?> attrs = font.getAttributes(); for (TextAttribute ta :
			 * attrs.keySet()) { System.out.println(ta + " -> " + attrs.get(ta)); }
			 */
		}
...
//      return m2;
			return (float) metrics.getStringBounds(buffer, start, stop, this.g2).getWidth();
		}
...
//    if (font != null && (textFont.isStream() || hints[ENABLE_NATIVE_FONTS])) {
		if (font != null) {
			 ...
			 * controlled when the font is created, not now as it's drawn. // fixed a bug in
			 * 0116 that handled this incorrectly. Object textAntialias =
			 * g2.getRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING);
			 * 
			 * // override the current text smoothing setting based on the font // (don't
			 * change the global smoothing settings)
			 * g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, textFont.smooth ?
			...
			this.g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
					this.textFont.isSmooth() ? RenderingHints.VALUE_ANTIALIAS_ON : RenderingHints.VALUE_ANTIALIAS_OFF);
...
//  }
	 ...
	 * Toolkit toolkit;
	 * 
	 * @SuppressWarnings("deprecation") protected FontMetrics getFontMetrics(Font
	 * font) { if (toolkit == null) { try { Canvas canvas = (Canvas)
	 * surface.getNative(); toolkit = canvas.getToolkit(); } catch (Exception e) {
	 * // May error out if it's a PSurfaceNone or similar toolkit =
	 * Toolkit.getDefaultToolkit(); } } return toolkit.getFontMetrics(font);
	...
	public void shearX(float angle) {
		this.g2.shear(Math.tan(angle), 0);
	}

	@Override
	public void shearY(float angle) {
		this.g2.shear(0, Math.tan(angle));
	}
	...
	public PMatrix2D getMatrix(PMatrix2D target) {
		...
		target.set((float) this.transform[0], (float) this.transform[2], (float) this.transform[4],
				(float) this.transform[1], (float) this.transform[3], (float) this.transform[5]);
		return target;
	}
	...
	public void setMatrix(PMatrix2D source) {
		this.g2.setTransform(
				new AffineTransform(source.m00, source.m10, source.m01, source.m11, source.m02, source.m12));
	}
	...
	protected void strokeImpl() {
		...
		this.strokeObject = new BasicStroke(this.strokeWeight, cap, join);
		this.g2.setStroke(this.strokeObject);
	}
...
//      }
			...
			this.pushMatrix();
			this.resetMatrix();
			this.g2.setColor(bgColor); // , backgroundAlpha));
//      g2.fillRect(0, 0, width, height);
	...
	protected WritableRaster getRaster() {
		...
		if (this.primaryGraphics) {
			/*
			 * // 'offscreen' will probably be removed in the next release if (useOffscreen)
			 * { raster = offscreen.getRaster(); } else
	...
	public void loadPixels() {
		...
		WritableRaster raster = this.getRaster();
		raster.getDataElements(0, 0, this.pixelWidth, this.pixelHeight, this.pixels);
...
//  }
	 ...
	 * Update the pixels[] buffer to the PGraphics image.
	 * <P>
	 * Unlike in PImage, where updatePixels() only requests that the update happens,
	 * in PGraphicsJava2D, this will happen immediately.
	 */
	@Override
	public void updatePixels(int x, int y, int c, int d) {
		// if ((x == 0) && (y == 0) && (c == width) && (d == height)) {
//    System.err.format("%d %d %d %d .. w/h = %d %d .. pw/ph = %d %d %n", x, y, c, d, width, height, pixelWidth, pixelHeight);
		if ((x != 0) || (y != 0) || (c != this.pixelWidth) || (d != this.pixelHeight)) {
...
//    updatePixels();
		if (this.pixels != null) {
			this.getRaster().setDataElements(0, 0, this.pixelWidth, this.pixelHeight, this.pixels);
		}
		this.modified = true;
	}
...
//      ((BufferedImage) (useOffscreen && primarySurface ? offscreen : image)).getRaster();
		...
		if ((sourceX == 0) && (sourceY == 0) && (sourceWidth == sourceImage.pixelWidth)
				&& (sourceHeight == sourceImage.pixelHeight)) {
//      System.out.format("%d %d  %dx%d  %d%n", targetX, targetY,
//                             sourceImage.width, sourceImage.height,
//                             sourceImage.pixels.length);
			raster.setDataElements(targetX, targetY, sourceImage.pixelWidth, sourceImage.pixelHeight,
					sourceImage.pixels);
		} else {
			// TODO optimize, incredibly inefficient to reallocate this much memory
