...
//  PGraphics graphics;  // moved to PSurfaceNone
	...
	public PSurfaceAWT(PGraphics graphics) {
		 ...
		 * 
		 * // The active-mode rendering seems to be 2x slower, so disable it // with
		 * retina. On a non-retina machine, however, useActive seems // the only (or
		 * best) way to handle the rendering. // useActive = false; // canvas = new
		 * JPanel(true) { // @Override // public void paint(Graphics screen) { //// if
		 * (!sketch.insideDraw) { // screen.drawImage(PSurfaceAWT.this.graphics.image,
		 * 0, 0, sketchWidth, sketchHeight, null); //// } // } // }; // Under 1.8 and
		 * the current 3.0a6 threading regime, active mode w/o // strategy is far
		 * faster, but perhaps only because it's blitting with // flicker--pushing
		...
		this.canvas.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				if (!PSurfaceAWT.this.sketch.isLooping()) {
					...
					if ((canvasSize.width != PSurfaceAWT.this.sketch.sketchWidth())
							|| (canvasSize.height != PSurfaceAWT.this.sketch.sketchHeight())) {
...
//  }
	...
	public class SmoothCanvas extends Canvas {
		...
		public SmoothCanvas() {
			super();
			this.setPreferredSize(new Dimension(PSurfaceAWT.this.sketchWidth, PSurfaceAWT.this.sketchHeight));
...
//        System.out.println("validate() render old=" + oldSize + " -> new=" + newSize);
				...
				PSurfaceAWT.this.sketch.setSize(this.newSize.width / PSurfaceAWT.this.windowScaleFactor,
						this.newSize.height / PSurfaceAWT.this.windowScaleFactor);
//        try {
...
//      if (useStrategy) {
			PSurfaceAWT.this.render();
			/*
			 * if (graphics != null) { System.out.println("drawing to screen " + canvas);
...
//      }
	...
	/*
	 * @Override public void addNotify() { // System.out.println("adding notify");
	 * super.addNotify(); // prior to Java 7 on OS X, this no longer works [121222]
...
//        try {
	...
	/*
	 * protected void blit() { // Other folks that call render() (i.e. paint()) are
	 * already on the EDT. // We need to be using the EDT since we're messing with
	 * the Canvas // object and BufferStrategy and friends.
	 * //EventQueue.invokeLater(new Runnable() { //public void run() {
	 * //((SmoothCanvas) canvas).render(); //} //});
	 * 
	 * if (useStrategy) { // Not necessary to be on the EDT to update BufferStrategy
	 * //((SmoothCanvas) canvas).render(); render(); } else { if (graphics.image !=
	 * null) { BufferedImage graphicsImage = (BufferedImage) graphics.image; if
	 * (offscreen == null || offscreen.getWidth() != graphicsImage.getWidth() ||
	 * offscreen.getHeight() != graphicsImage.getHeight()) {
	 * System.out.println("creating new image"); offscreen = (BufferedImage)
	 * canvas.createImage(graphicsImage.getWidth(), graphicsImage.getHeight()); //
	 * off = offscreen.getGraphics(); } // synchronized (offscreen) { Graphics2D off
	 * = (Graphics2D) offscreen.getGraphics(); //
	 * off.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1));
	 * off.drawImage(graphicsImage, 0, 0, null); // } off.dispose(); synchronized
	 * (offscreenLock) { BufferedImage temp = onscreen; onscreen = offscreen;
	 * offscreen = temp; } canvas.repaint(); } } }
	 */
	...
	/*
	 * public Frame initOffscreen() { Frame dummy = new Frame(); dummy.pack(); //
	 * get legit AWT graphics // but don't show it return dummy; }
	 */

	/*
	 * @Override public Component initComponent(PApplet sketch) { this.sketch =
	 * sketch;
	 * 
	 * // needed for getPreferredSize() et al sketchWidth = sketch.sketchWidth();
	...
	public void initFrame(final PApplet sketch) {/*
													 * , int backgroundColor, int deviceIndex, boolean fullScreen,
...
//    System.out.println("display from sketch is " + displayNum);
		...
		this.screenRect = spanDisplays ? getDisplaySpan() : this.displayDevice.getDefaultConfiguration().getBounds();
		...
		this.windowScaleFactor = PApplet.platform == PConstants.MACOSX ? 1 : sketch.pixelDensity;

		this.sketchWidth = sketch.sketchWidth() * this.windowScaleFactor;
		this.sketchHeight = sketch.sketchHeight() * this.windowScaleFactor;
...
//    }
		...
		this.setSize(this.sketchWidth / this.windowScaleFactor, this.sketchHeight / this.windowScaleFactor);
		this.frame.setSize(
				new Dimension(this.sketchWidth / this.windowScaleFactor, this.sketchHeight / this.windowScaleFactor));
		/*
		 * if (fullScreen) { // Called here because the graphics device is needed before
		 * we can // determine whether the sketch wants size(displayWidth,
		 * displayHeight), // and getting the graphics device will be PSurface-specific.
		 * PApplet.hideMenuBar();
		 * 
		 * // Tried to use this to fix the 'present' mode issue. // Did not help, and
		 * the screenRect setup seems to work fine.
		 * //frame.setExtendedState(Frame.MAXIMIZED_BOTH);
		 * 
		 * // https://github.com/processing/processing/pull/3162 frame.dispose(); //
		 * release native resources, allows setUndecorated() frame.setUndecorated(true);
		 * // another duplicate? // if (backgroundColor != null) { //
		 * frame.getContentPane().setBackground(backgroundColor); // } // this may be
		 * the bounds of all screens frame.setBounds(screenRect); // will be set visible
		 * in placeWindow() [3.0a10] //frame.setVisible(true); // re-add native
		 * resources }
		 */
...
//  }
	...
	protected void setProcessingIcon(Frame frame) {
		...
		} else if (!this.dockIconSpecified()) { // don't override existing -Xdock param
	...
	public void setVisible(boolean visible) {
		...
		/*
		 * if (visible && PApplet.platform == PConstants.LINUX) { // Linux doesn't deal
		 * with insets the same way. We get fake insets // earlier, and then the window
		 * manager will slap its own insets // onto things once the frame is realized on
		 * the screen. Awzm. if (PApplet.platform == PConstants.LINUX) { Insets insets =
		 * frame.getInsets(); frame.setSize(Math.max(sketchWidth, MIN_WINDOW_WIDTH) +
		 * insets.left + insets.right, Math.max(sketchHeight, MIN_WINDOW_HEIGHT) +
...
//    frame.setBounds(screenRect);  // already called in setFullFrame()
		this.canvas.setBounds((this.screenRect.width - this.sketchWidth) / 2,
				(this.screenRect.height - this.sketchHeight) / 2, this.sketchWidth, this.sketchHeight);
...
//    }
	 ...
	 * screenRect.y + (screenRect.height - sketchHeight) / 2); } Point frameLoc =
	 * frame.getLocation(); if (frameLoc.y < 0) { // Windows actually allows you to
	 * place frames where they can't be // closed. Awesome.
	 * http://dev.processing.org/bugs/show_bug.cgi?id=1508
	 * frame.setLocation(frameLoc.x, 30); }
	 * 
	 * // if (backgroundColor != null) { // ((JFrame)
	 ...
	 * 
	 * // handle frame resizing events setupFrameResizeListener();
	 * 
	 * // all set for rockin if (sketch.getGraphics().displayable()) {
...
//    new Exception().printStackTrace(System.out);
		int contentW = Math.max(this.sketchWidth, MIN_WINDOW_WIDTH);
		int contentH = Math.max(this.sketchHeight, MIN_WINDOW_HEIGHT);

		this.canvas.setBounds((contentW - this.sketchWidth) / 2, (contentH - this.sketchHeight) / 2, this.sketchWidth,
				this.sketchHeight);
	}
	...
	private Dimension setFrameSize() { // int sketchWidth, int sketchHeight) {
		...
		this.frame.addNotify(); // using instead of show() to add the peer [fry]

//    System.out.format("setting frame size %d %d %n", sketchWidth, sketchHeight);
//    new Exception().printStackTrace(System.out);
		this.currentInsets = this.frame.getInsets();
		int windowW = Math.max(this.sketchWidth, MIN_WINDOW_WIDTH) + this.currentInsets.left + this.currentInsets.right;
		int windowH = Math.max(this.sketchHeight, MIN_WINDOW_HEIGHT) + this.currentInsets.top
	...
	private void setFrameCentered() {
		...
		this.frame.setLocation(this.screenRect.x + ((this.screenRect.width - this.sketchWidth) / 2),
	...
	public void placeWindow(int[] location, int[] editorLocation) {
		...
		Dimension window = this.setFrameSize(); // sketchWidth, sketchHeight);
		...
		if (!this.sketch.sketchFullScreen()) {
			if (location != null) {
				...
				if ((locationX - window.width) > 10) {
					...
					/*
					 * if ((locationX + window.width > sketch.displayWidth - 33) || (locationY +
					 * window.height > sketch.displayHeight - 33)) { // otherwise center on screen
					 * locationX = (sketch.displayWidth - window.width) / 2; locationY =
					 * (sketch.displayHeight - window.height) / 2; }
					 */
		 ...
		 * System.out.println("setting visible on EDT? " +
		 * EventQueue.isDispatchThread()); //requestFocus(); // if (canvas != null) { //
		 * //canvas.requestFocusInWindow(); // canvas.requestFocus(); // } }
		 */
//    if (sketch.getGraphics().displayable()) {
//      setVisible(true);
//    }
...
//    }
		...
		if ((wide == this.sketch.width) && (high == this.sketch.height)
				&& ((this.frame == null) || this.currentInsets.equals(this.frame.getInsets()))) {
//      if (PApplet.DEBUG) {
...
//    frame.setSize(wide, high);
		if (this.frame != null) { // skip if just a canvas
			this.setFrameSize(); // wide, high);
		}
		this.setCanvasSize();
//    if (frame != null) {
...
//    System.out.println("out of setSize()");
	...
	/*
	 * @Override public void initImage(PGraphics graphics) { GraphicsConfiguration
	 * gc = canvas.getGraphicsConfiguration(); // If not realized (off-screen, i.e
	 * the Color Selector Tool), gc will be null. if (gc == null) {
	 * System.err.println("GraphicsConfiguration null in initImage()");
	 * GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
	 * gc = ge.getDefaultScreenDevice().getDefaultConfiguration(); }
	 * 
	 * // Formerly this was broken into separate versions based on offscreen or //
	 * not, but we may as well create a compatible image; it won't hurt, right? int
	 * wide = graphics.width * graphics.pixelFactor; int high = graphics.height *
	 * graphics.pixelFactor; graphics.image = gc.createCompatibleImage(wide, high);
	 * }
	 */
...
//  }
	...
	/*
	 * private boolean checkRetina() { if (PApplet.platform == PConstants.MACOSX) {
	 * // This should probably be reset each time there's a display change. // A
	 * 5-minute search didn't turn up any such event in the Java 7 API. // Also,
	 * should we use the Toolkit associated with the editor window? final String
	 * javaVendor = System.getProperty("java.vendor"); if
	 ...
	 * 
	 * try { Field field = device.getClass().getDeclaredField("scale"); if (field !=
	 * null) { field.setAccessible(true); Object scale = field.get(device);
	 * 
	 * if (scale instanceof Integer && ((Integer)scale).intValue() == 2) { return
	 * true; } } } catch (Exception ignore) { } } } return false; }
	 */
	...
	/*
	 * private void checkDisplaySize() { if (canvas.getGraphicsConfiguration() !=
	 * null) { GraphicsDevice displayDevice =
	 ...
	 * <p/>
	 * This uses the stderr stream to write positions of the window (so that it will
	 * be saved by the PDE for the next run) and notify on quit. See more notes in
	 * the Worker class.
	 */
	@Override
		...
		this.frame.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				...
				if (farm.isVisible()) {
					...
					int w = windowSize.width - PSurfaceAWT.this.currentInsets.left
							- PSurfaceAWT.this.currentInsets.right;
					int h = windowSize.height - PSurfaceAWT.this.currentInsets.top
							- PSurfaceAWT.this.currentInsets.bottom;
					PSurfaceAWT.this.setSize(w / PSurfaceAWT.this.windowScaleFactor,
							h / PSurfaceAWT.this.windowScaleFactor);

					// correct the location when inset size changes
					PSurfaceAWT.this.setLocation(x - PSurfaceAWT.this.currentInsets.left,
							y - PSurfaceAWT.this.currentInsets.top);
				}
			}
		});
	}
...
//  }
	 ...
	 * calling ceil() on the value: 1.5 goes to 1, but 2.3 goes to 2. // "precise"
	 * is a whole different animal, so add later API to shore that up. static
	 * protected Method preciseWheelMethod; static { try { preciseWheelMethod =
	 * MouseWheelEvent.class.getMethod("getPreciseWheelRotation", new Class[] { });
	 * } catch (Exception e) { // ignored, the method will just be set to null } }
	 */
	...
	protected void nativeMouseEvent(java.awt.event.MouseEvent nativeEvent) {
		...
		case java.awt.event.MouseEvent.MOUSE_WHEEL:
			 ...
			 * preciseWheelMethod.invoke(nativeEvent, (Object[]) null)).floatValue(); }
			 * catch (Exception e) { preciseWheelMethod = null; } }
			 */
...
//    }
		...
		this.sketch.postEvent(new MouseEvent(nativeEvent, nativeEvent.getWhen(), peAction, peModifiers,
				nativeEvent.getX() / this.windowScaleFactor, nativeEvent.getY() / this.windowScaleFactor, peButton,
				peCount));
	}
...
//       InputEvent.ALT_DOWN_MASK);
		...
		this.sketch.postEvent(
				new KeyEvent(event, event.getWhen(), peAction, peModifiers, event.getKeyChar(), event.getKeyCode()));
	}
	...
	/*
	 * public void addListeners(Component comp) { comp.addMouseListener(this);
	 * comp.addMouseWheelListener(this); comp.addMouseMotionListener(this);
	 * comp.addKeyListener(this); comp.addFocusListener(this); }
	 * 
	 * 
	 * public void removeListeners(Component comp) { comp.removeMouseListener(this);
	 * comp.removeMouseWheelListener(this); comp.removeMouseMotionListener(this);
	 * comp.removeKeyListener(this); comp.removeFocusListener(this); }
	 */
